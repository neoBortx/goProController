package com.bortxapps.goprocontrollerexample.screens.cameracontrol.commands.views
...
import com.bortxapps.goprocontrollerexample.screens.cameracontrol.commands.CameraCommandsScreenState
...
fun CameraCommandsScreen(viewModel: CameraCommandsViewModel = koinViewModel()) {
    ...
    Screen(
        ...
        onShutterClicked = { viewModel.intentChannel.trySend(CameraCommandsScreenIntent.ShutterClicked) }
            ...
            Column(modifier = Modifier.padding(paddingValues)) {
                state.let {
                    when (it) {
                        ...
                        is CameraCommandsScreenState.Error -> ErrorText(this, it.error)
...
private fun CommandsList(
    ...
    Column(
        ...
        PresetsConfiguration(state.presets, onPresetsChanged)

        ResolutionConfiguration(state.resolution, onResolutionChanged)

        FrameRateConfiguration(state.frameRate, onFrameRateChanged)

        HyperSmoothConfiguration(state.hyperSmooth, onHyperSmoothChanged)

        SpeedConfiguration(state.speed, onSpeedChanges)

        Button(onClick = { onShutterClicked() }, modifier = Modifier.padding(16.dp)) {
            Text(text = "Shutter")
        }
    }
}
...
@Composable
fun PresetsConfiguration(presets: Presets, onPresetsChanged: (Presets) -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        IconButton(onClick = { showPresetsMenu = !showPresetsMenu }, modifier = Modifier.fillMaxWidth()) {
            Row {
                ...
                Text(text = presets.toString(), Modifier.fillMaxWidth())
            }
        }
...
@Composable
fun HyperSmoothConfiguration(hyperSmooth: HyperSmooth, onHyperSmoothChanged: (HyperSmooth) -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        IconButton(onClick = { showHyperSmoothMenu = !showHyperSmoothMenu }, modifier = Modifier.fillMaxWidth()) {
            Row {
                Icon(Icons.Default.ArrowDropDown, "")
                Text(text = hyperSmooth.toString(), Modifier.fillMaxWidth())
            }
        }
...
@Composable
fun ResolutionConfiguration(resolution: Resolution, onResolutionChanged: (Resolution) -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        IconButton(onClick = { showResolutionMenu = !showResolutionMenu }, modifier = Modifier.fillMaxWidth()) {
            Row {
                Icon(Icons.Default.ArrowDropDown, "")
                Text(text = resolution.toString(), Modifier.fillMaxWidth())
            }
        }
...
@Composable
fun FrameRateConfiguration(frameRate: FrameRate, onFrameRateChanged: (FrameRate) -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        IconButton(onClick = { showFramesMenu = !showFramesMenu }, modifier = Modifier.fillMaxWidth()) {
            Row {
                Icon(Icons.Default.ArrowDropDown, "")
                Text(text = frameRate.toString(), Modifier.fillMaxWidth())
            }
        }
...
@Composable
fun SpeedConfiguration(speed: Speed, onSpeedChanges: (Speed) -> Unit) {
    ...
    Box(modifier = Modifier.fillMaxWidth()) {
        IconButton(onClick = { showSpeedMenu = !showSpeedMenu }, modifier = Modifier.fillMaxWidth()) {
            Row {
                Icon(Icons.Default.ArrowDropDown, "")
                Text(text = speed.toString(), Modifier.fillMaxWidth())
            }
        }
